classdef RM_RN < handle
    ...
    methods (Access = public)
        ...
        function self = RM_RN(varargin)
            ...
            self.ricker = @(In,omega,delta)...
                (2*delta/(sqrt(3*omega)*pi^0.25)) * exp(-In.^2/(2*omega^2)) .* (1-(In./omega).^2);
            ...
            self.Theta = linspace(-30,30,self.N)';
            self.mean_JND =  0;
            self.V_0 = zeros(self.N,1);
            % initial recurrent weight matrix
            self.W_rec = self.J_rec * toeplitz(...
                self.ricker(self.Theta, self.omega, self.delta));
            self.Phi_0 = 135;
            self.Phi = self.Phi_0;
        ...
        function reset(self)
            self.W_rec = toeplitz(self.ricker(...
                (self.Theta)', self.omega, self.delta));
            self.Eta = self.eta*ones(self.N);
            self.OD = self.OD_0;
        end
            ...
            if ~correct
                if rand() < (1 - self.p_correct)
                    ...
                    dW_rec = self.eta*((1-self.W_rec/...
                        (self.J_rec*self.omega*2^self.delta)).^self.mu).*...
                        (r*r');
